  ...
  return (
    <div className="space-y-4 mb-6">
      <div className="flex items-center gap-2 px-6">
        ...
        <h3 className="text-white font-medium">Search Results for "{query}"</h3>
        <span className="text-white/60 text-sm">({results.length} tweets)</span>
      </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0">
                {result.profilePicture ? (
                  <img
                    ...
                    alt={`${result.author} profile picture`}
                    ...
                    onError={(e) => {
                      ...
                      if (parent) {
                        ...
                        parent.innerHTML = `<span class="text-white font-semibold text-sm">${result.author.charAt(0).toUpperCase()}</span>`;
                      }
                    }}
                  />
                ) : (
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-white font-medium">{result.author}</span>
                  <span className="text-white/60 text-sm">@{result.authorUsername}</span>
                  <span className="text-white/40 text-sm">·</span>
                  <span className="text-white/60 text-sm">{formatDate(result.createdAt)}</span>
                </div>
              </div>
              <a
                href={result.url}
